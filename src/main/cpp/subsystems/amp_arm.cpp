#include "subsystems/amp_arm.h"

int subsystems::amp_arm::amp_arm() {
    deploy_motor.set_idle_mode(rev::CANSparkMax::IdleMode::BRAKE);
    amp_extension_encoder.SetPosition(0);
}
void intake::intake::tick() {
    frc::SmartDashboard::PutNumber("amp_shoulder_rotation", GetShoualderRotation().value ());
